...
const TableTwoChairs = ({name, id, reservedChairs, hover, onClick}) => {
    ...
    return (
        <div id={id} className=" m-10 w-[180px] h-[180px]">
            ...
            <div className={`${style} bg-[#C0C0C030] w-[130px] h-[130px] rotate-45 border-solid rounded-xl relative top-[-155px] left-[25px] backdrop-blur border-4 border-[#343333]`} onClick={onClick}>
                <div className="w-full h-full flex justify-center items-center -rotate-45">
                    <span className="text-xl font-title text-color-secondary font-bold text-center">{name}</span>
                </div>
            </div>
        </div>
    )

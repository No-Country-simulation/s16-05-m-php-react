...
const ProductClient = () => {
    const {product, error, loading, fetchProduct } = useProductStore();
    ...
    if (error) {
        return (
            <div className="flex justify-center text-center w-full bg-color-bg h-[100vh]">
                {error}
            </div>
        );
    }
    ...
    if (!product) {
        return (
            <div className="flex justify-center w-full h-[100vh] text-center bg-color-bg">
            ...
            <div className="flex justify-center w-full my-5">
                <div className="w-4/5 flex justify-around max-w-80 min-w-48 items-center bg-[#7C7676] rounded-2xl p-1">
                    <img
                        ...
                        className="w-16 h-16"
                    />
                    <p className="text-[white] text-center text-wrap"> {restaurantName} Restaurant</p>
                    <img src={pet} alt="Se admiten mascotas" className="w-8 h-8" />
                </div>
            </div>
            <div className="flex justify-center w-full">
                <CarouselComponent data={product} solicitud={"product"}/>
            </div>
            <div className="flex justify-center w-full my-5">
                <Button1 text={"Volver"} onClick={handleClick} />
            </div>
        </div>
    );
};
